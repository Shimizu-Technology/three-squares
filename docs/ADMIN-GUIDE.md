...
Three Squares uses a 4-level role system. Assign the right role so each person only sees what they need:

| Role | Who it's for | What they can do |
|------|-------------|-----------------|
| **Admin** | Marie, Liv (managers/owners) | Full access — menu, settings, staff, reports, POS |
| **Manager** | Shift supervisors | Menu editing, order management, POS, staff view (cannot change settings or add admins) |
| **Staff** | Counter/kitchen staff | Process orders, use POS only — cannot edit menu or settings |
| **Customer** | Regular customers | Browse menu, place orders only |

> **Recommendation for B&G:** Marie = Admin, Liv = Admin, counter staff = Staff.
...
4. Select their role (see table above):
   - **Admin** — for owners and top managers
   - **Manager** — for shift supervisors (can edit menu, not settings)
   - **Staff** — for counter and kitchen staff (order processing + POS only)
5. If assigning **Staff** or **Manager**, select their **location** (Donki, Three Squares, etc.)
6. They'll receive an invitation email to create their account

### Changing Someone's Role

1. Go to **Admin → Staff**
2. Find the person → click the **pencil icon**
3. Change their role in the dropdown
4. Click **Save**

> **Note:** Admins can only assign roles at or below their own level. An Admin cannot create another Admin (only a Super Admin can do that — contact Leon if needed).
